...
export class HEntityUpdate {
    ...
    [util.inspect.custom](depth) {
        ...
        + `${indent}  jumpingPower: ${util.inspect(this.#jumpingPower, false, depth + 1)}\n`
        + `${indent}}`;
    }
    ...
    constructor(packet) {
        ...
        let action;
        [ this.#headFacing, this.#bodyFacing, this.#jumpingPower, action ] = packet.read('iiiS');
    ...
    get jumpingPower() {
        return this.#jumpingPower;
    }

    set jumpingPower(val) {
        if (!Number.isInteger(val)) {
            throw new Error('HEntityUpdate.jumpingPower: must be an integer')
        }

        this.#jumpingPower = val;
    }
}